 ...
 */
class RaidStage extends AppModel {
 ...
 * @var string
 */
	public $primaryKey = 'raid_stage_id';
 ...
 */
    ...
    public function getLastStageId($questId) {
        ...
        $kind = 'first';
        $where = array('raid_quest_id' => $questId);
        $order = array('raid_stage_id DESC');

        $data = $this->getAllFind($where, $fields, $kind, $order, $limit = 1);
        if (empty($data['raid_stage_id'])) return false;
        return $data['raid_stage_id'];
    }
    ...
    public function getStageIdByEnemyId($enemyId) {
        ...
        $where = array('RaidStage.enemy_id' => $enemyId);

        $evStageId = $this->field('raid_stage_id', $where);
