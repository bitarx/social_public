...
class EvStagesController extends ApiController {
    ...
    public function beforeFilter(){
        ...
        if (!empty($this->params['ev_quest_id'])) {
            if ($this->params['ev_quest_id'] != $this->event['ev_quest_id']) {
                 $this->log( __FILE__ .  ':' . __LINE__ .' : Event Not Exist :userId:' . $this->userId );
                 $this->rd('errors', 'index', array('error' => ERROR_ID_BAD_OPERATION )); 
            }
        }
        if (!empty($this->params['ev_stage_id'])) {
                    
             $stageList = $this->EvStage->getQuestStageId($this->event['ev_quest_id']);
             if (empty($stageList)) {
                 $this->log( __FILE__ .  ':' . __LINE__ .' : Event Not Exist :userId:' . $this->userId );
                 $this->rd('errors', 'index', array('error' => ERROR_ID_BAD_OPERATION )); 
             } 
             ...
             foreach ($stageList as $val) {
                 if ($val['ev_stage_id'] == $this->params['ev_stage_id']) 
                     $ret = true;
             }
             if (empty($ret)) {
                 $this->log( __FILE__ .  ':' . __LINE__ .' : Event Not Exist :userId:' . $this->userId );
	...
	public function index() {
        ...
        if (empty($ret)) {
            ...
            // 現在のクエストステージを抽出
            foreach ($ret as $val)  {
                if ($val['ev_quest_id'] == $questId) {
                    $list[] = $val;
                }
            }
            $questData = $this->EvQuest->getQuestData($questId);
	...
	public function initStage() {
        ...
        try {
            if ($first == 1) {
                ...
                $this->EvUserStage->save($values);    
            }
            $values = array(
                'user_id' => $this->userId  
            ,   'ev_stage_id' => $stageId
            );
            $this->EvUserCurStage->save($values);    
        } catch (AppException $e) { 
            $this->EvUserCurStage->rollback(); 
            $this->log($e->errmes);
            $this->rd('Errors', 'index', array('error'=> ERROR_ID_SYSTEM )); 
        } 
        $this->EvUserCurStage->commit(); 

        $params = array('ev_stage_id' => $stageId);
        $this->rd('EvStages', 'main', $params);
    }
    ...
    public function main() {
        ...
        $data = $this->EvUserStage->getUserStage($this->userId, $stageId, $recu = 2);
    ...
    public function conf() {
        ...
        $userStageData = $this->EvUserStage->getUserStage($this->userId, $stageId);
    ...
    public function act() {
        ...
        $userStage = $this->EvUserStage->getUserStageByEnemyId($this->userId, $targetId ,$state = 2);
        // ブラウザバックなど不正操作
        if (empty($userStage) || 2 != $userStage['EvUserStage']['state']) {
        ...
        if (empty($userCards)) {
             $this->log( __FILE__ .  ':' . __LINE__ .':userId:' . $this->userId );
             $this->rd('Stages', 'conf', array('nocard' => 1, 'ev_stage_id'=> $stageId));
        }
        ...
        try {  
            ...
            $this->EvBattleLog->registBattleLog($values);

            // 勝利の場合ステータス変更
            if ($winner == 1) {
                $where = array('enemy_id' => $targetId);
                $field = array('ev_stage_id');
                $data = $this->EvStage->getAllFind($where, $field, 'first');
    ...
    public function product() {
        ...
        $data = $this->EvBattleLog->getBattleLogDataLatest($this->userId);
    ...
    public function scene() {
        ...
        $this->set('data', $data);
        $this->set('next', $next);
        $this->set('questId', $stage['EvStage']['ev_quest_id']);
    }
    ...
    public function comp() {
        ...
        $this->EvUserStage->begin(); 
        try {  

            $values = array(
                          'id' => $log['id']
                      );
            $this->EvBattleLog->save($values);
        
        } catch (AppException $e) { 
            $this->EvUserStage->rollback(); 
            $this->log($e->errmes);
            return $this->rd('Errors', 'index', array('error'=> ERROR_ID_SYSTEM )); 
        } 
	...
	public function init() {
        ...
        if ('waku' == PLATFORM_ENV || $this->request->is(array('ajax'))) {
            ...
            // 現在のユーザステージ情報取得
            $userStageData = $this->EvUserStage->getUserStage($userId, $data['ev_stage_id']);
            ...
            if ($hit <= $userStageData['prob_get']) {
                // 抽選設定値取得
                $list = $this->EvStageProb->getStageProb($data['ev_stage_id']);
            ...
            try {
                ...
                if ($data['progress'] < 100) {
                    ...
                    if (100 <= $data['progress']) { 
                        ...
                        $this->EvUserStage->initUserStage($data);    
                    }
                }
                ...
                ,   'ev_stage_id'         => $data['ev_stage_id']         // ステージID
